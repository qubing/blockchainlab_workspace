...
func Test_Init(t *testing.T) {
	...
	stub := shim.NewMockStub("example02", mycc)
	response := stub.MockInit("init", util.ArrayToChaincodeArgs([]string{"a"}))
	if response.Status != shim.ERROR {
		t.Fail()
	}

	response = stub.MockInit("init", util.ArrayToChaincodeArgs([]string{"init", "a", "100", "b", "100"}))
...
func Test_Invoke_query_a(t *testing.T) {
	...
	stub.MockInit("init", util.ArrayToChaincodeArgs([]string{"init", "a", "100", "b", "100"}))
	...
	response := stub.MockInvoke("invoke", util.ArrayToChaincodeArgs([]string{"transfer", "a", "b", "10"}))
	if response.Status != shim.OK {
