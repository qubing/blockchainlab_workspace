...
export CHANNEL_FILE=./channel-artifacts/channel.tx

peer channel create -o orderer.example.com:7050 -c mychannel -f $CHANNEL_FILE --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_TLS_ROOTCERT
peer channel join -b mychannel.block
peer channel update -o orderer.example.com:7050 -c mychannel -f ./channel-artifacts/Org1MSPanchors.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_TLS_ROOTCERT
peer chaincode install -n mycc -v 1.0 -l golang -p github.com/chaincode/chaincode_example02_v1.0/go/
peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_TLS_ROOTCERT -C mychannel -n mycc -l golang -v 1.0 -c '{"Args":["init","a","100","b","200"]}'
sleep 5
peer chaincode query -C mychannel -n mycc -c '{"Args":["query","a"]}'

#peer chaincode invoke -o orderer.example.com:7050 --tls true --cafile /opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem -C mychannel -n mycc 
...
peer chaincode invoke -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_TLS_ROOTCERT -C mychannel -n mycc --peerAddresses peer0.org1.example.com:7051 --tlsRootCertFiles $CORE_PEER_TLS_ROOTCERT_FILE -c '{"Args":["transfer","a","b","10"]}'
sleep 3
peer chaincode query -C mychannel -n mycc -c '{"Args":["query","a"]}'
